...
export function getPanelMenu(
  ...
  // 退出全屏
  const exitViewPanel = (event: React.MouseEvent<any>) => {
    event.preventDefault();
  ...
  if (!panel.isEditing) {
    ...
    menu.push({
      text: !search.viewPanel ? '全屏查看' : '退出全屏',
      iconClassName: 'eye',
      onClick: !search.viewPanel ? onViewPanel : exitViewPanel,
      shortcut: 'v',
    });
  }

  if (dashboard.meta.canAdmin && dashboard.canEditPanel(panel) && !panel.isEditing) {
    menu.push({
      text: 'Edit',
      iconClassName: 'edit',
      onClick: onEditPanel,
  ...
  // 分享链接时隐藏
  if (dashboard.meta.canAdmin && dashboard.canEditPanel(panel) && !(panel.isViewing || panel.isEditing)) {
    subMenu.push({
      text: 'Duplicate',
  ...
  // 分享链接时隐藏
  if (dashboard.meta.canAdmin && !panel.isEditing && subMenu.length) {
    menu.push({
      type: 'submenu',
      text: 'More...',
      iconClassName: 'cube',
  ...
  // 分享链接时隐藏
  if (dashboard.meta.canAdmin && dashboard.canEditPanel(panel) && !panel.isEditing && !panel.isViewing) {

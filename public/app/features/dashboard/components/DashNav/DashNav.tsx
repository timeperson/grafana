...
export const DashNav = React.memo<Props>((props) => {
  ...
  const renderLeftActionsButton = () => {
    ...
    const { canStar, canShare, isStarred, canAdmin } = dashboard.meta;
  ...
  const renderRightActionsButton = () => {
    ...
    const { canAdmin, canSave, canEdit, showSettings } = dashboard.meta;
    ...
    if (canAdmin && canEdit && !isFullscreen) {
      buttons.push(<ToolbarButton tooltip="Add panel1" icon="panel-add" onClick={onAddPanel} key="button-panel-add" />);
    }

    ...
    return (
      <div className={styles.wrap}>
        <div aria-label={selectors.components.PanelInspector.Query.content}>
          <h3 className="section-heading">查询检查</h3>
          <p className="small muted">
            查询检查器允许您查看原始请求和响应。为了收集这些数据，Grafana需要发出
            一个新的查询。单击下面的刷新按钮以触发新的查询。
          </p>
        </div>
        ...
        <div className={styles.content}>
          {isLoading && <LoadingPlaceholder text="加载查询检查..." />}
          {!isLoading && haveData && (
            <JSONFormatter json={response} open={openNodes} onDidRender={this.setFormattedJson} />
          )}
          {!isLoading && !haveData && <p className="muted">还没有收集到请求和响应。点击刷新按钮</p>}
        </div>
      </div>
    );
  }

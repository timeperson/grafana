...
export class UnthemedLokiLabelBrowser extends React.Component<BrowserProps, BrowserState> {
  ...
  async validateSelector(selector: string) {
    ...
    this.setState({ validationStatus: `选择器有效 (找到${streams.length}个数据流)` });
  }
  ...
  render() {
    ...
    if (labels.length === 0) {
      return <LoadingPlaceholder text="标签加载中..." />;
    }
    ...
    return (
      <div className={styles.wrapper}>
        <div className={styles.section}>
          <Label description="您希望在搜索时考虑哪些标签？">1. 选择要搜索的标签</Label>
        ...
        <div className={styles.section}>
          <Label description="选择要用于查询的标签值。使用搜索字段查找所选标签的值。">2. 查找所选标签的值</Label>
          <div>

...
export const LIMIT_LABEL = '行限制';
export const COMMON_LABELS = '通用标签';
...
function adjustMetaInfo(logsModel: LogsModel, visibleRangeMs?: number, requestedRangeMs?: number): LogsMetaItem[] {
  ...
  if (limit && limit > 0) {
    ...
    if (limit === logsModel.rows.length && visibleRangeMs && requestedRangeMs) {
      ...
      metaLimitValue = `收到的 ${limit} 条日志, 覆盖了您所选时间范围(${rangeUtil.msRangeToTimeString(
        requestedRangeMs
      )})的 ${coverage}% (${rangeUtil.msRangeToTimeString(visibleRangeMs)})`;
    } else {

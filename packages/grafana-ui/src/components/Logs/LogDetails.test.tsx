...
describe('LogDetails', () => {
  describe('when labels are present', () => {
    it('should render heading', () => {
      ...
      expect(screen.getAllByLabelText('日志标签')).toHaveLength(1);
    });
  ...
  describe('when row entry have parsable fields and labels are present', () => {
    it('should render all headings', () => {
      ...
      expect(screen.getAllByLabelText('日志标签')).toHaveLength(1);
      expect(screen.getAllByLabelText('检测到的字段')).toHaveLength(1);
    });
  ...
  describe('when row entry and labels are not present', () => {
    it('should render no details available message', () => {
      ...
      expect(screen.getByText('没有可用的详细信息')).toBeInTheDocument();
    });
    it('should not render headings', () => {
      setup(undefined, { entry: '' });
      expect(screen.queryAllByLabelText('日志标签')).toHaveLength(0);
      expect(screen.queryAllByLabelText('检测到的字段')).toHaveLength(0);
    });
  });

  ...
  render() {
    ...
    const toggleFieldButton =
      !isLabel && showDetectedFields && showDetectedFields.includes(parsedKey) ? (
        ...
        <IconButton name="eye" title="只显示这个字段" onClick={this.showField} />
      );
    ...
    return (
      <tr className={cx(style.logDetailsValue, { [styles.noHoverBackground]: showFieldsStats })}>
        ...
        {hasFilteringFunctionality && isLabel && (
          <>
            <td className={style.logsDetailsIcon}>
              <IconButton name="search-plus" title="筛选值" onClick={this.filterLabel} />
            </td>
            <td className={style.logsDetailsIcon}>
              <IconButton name="search-minus" title="排除值" onClick={this.filterOutLabel} />
            </td>
          </>
        )}
